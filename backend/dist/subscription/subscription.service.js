...
let SubscriptionService = SubscriptionService_1 = class SubscriptionService {
    db;
    log = new common_1.Logger(SubscriptionService_1.name);
    constructor(db) {
        this.db = db;
    }
    /**
    ...
    async ensure(userId) {
        ...
        await this.db.query(`INSERT INTO subscriptions (user_id, status, plan, daily_allowance, likes_remaining_today)
       VALUES ($1, 'active', 'free', $2, $2)
       ON CONFLICT (user_id) DO NOTHING`, [userId, allowance]);
        return (await this.db.one('SELECT * FROM subscriptions WHERE user_id = $1', [
            userId,
        ]));
    }
    async status(userId) {
     ...
     */
    async nightlyQuotaReset() {
        const lapsed = await this.db.query(`UPDATE subscriptions SET status = 'expired'
        WHERE status = 'active' AND renews_at IS NOT NULL AND renews_at < now()`);
        const reset = await this.db.query(`UPDATE subscriptions
     ...
     */
    async applyWebhook(event, secret) {
        const expected = process.env.REVENUECAT_WEBHOOK_SECRET;
        if (!expected || secret !== expected)
            throw new common_1.UnauthorizedException('invalid_webhook_secret');
        const user = await this.db.one('SELECT id FROM users WHERE id = $1', [
            event.app_user_id,
        ]);
        if (!user)
            return { applied: false, status: 'inactive' };
        ...
        await this.db.query(`UPDATE subscriptions
          SET status = $2,
              ...
              renews_at = $5
        WHERE user_id = $1`, [user.id, status, plan, ALLOWANCE_BY_PLAN[plan] ?? ALLOWANCE_BY_PLAN.free, renewsAt]);
...
], SubscriptionService.prototype, "nightlyQuotaReset", null);
